...
export const PlanCard = ({ plan }) => {
  ...
  return (
    <article className={`card relative flex h-full flex-col p-6 ${plan.is_featured ? 'ring-2 ring-primary' : ''}`}>
      {plan.is_featured ? <div className="absolute right-4 top-4"><Badge tone="pending">Pho bien nhat</Badge></div> : null}
      ...
      <div className="mt-5">
        ...
        <span className="text-sm text-zinc-500"> / {plan.duration_days} ngay</span>
      </div>
      ...
      <Button className="mt-6 w-full" onClick={() => add(plan.id)}><Plus size={17} />Them vao gio</Button>
    </article>
  );
};
