  ...
  return (
    <main className="container-page py-10">
      <div className="flex flex-col justify-between gap-5 md:flex-row md:items-end">
        <div><h1 className="text-4xl font-black">Doi ngu HLV</h1><p className="mt-2 text-zinc-500">Loc theo chuyen mon va xem review tu hoi vien.</p></div>
        <div className="flex flex-wrap gap-2">{specialties.map((s) => <button key={s} onClick={() => setSpecialty(s)} className={`rounded-lg px-3 py-2 text-sm font-bold ${specialty === s ? 'bg-primary text-black' : 'bg-white dark:bg-zinc-900'}`}>{s === 'all' ? 'Tat ca' : s}</button>)}</div>
      </div>
